...
void Tools::fillArrayRandom(int64_t *arr, const int64_t size, const int64_t minValue, const int64_t maxValue)
{ 
    #pragma omp parallel
    {
...
int64_t Tools::getRandomInt(const int64_t min, const int64_t max)
{
    const int64_t range = std::abs(max - min) + 1;
    ...
    bool useTimers)
{
    ...
    for (int64_t i = 0; i < iterations; ++i) {
        //Clear result
        std::memcpy(result, initArr, size * sizeof(int64_t));
